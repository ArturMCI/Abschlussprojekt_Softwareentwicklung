import numpy as np
from src.model import Structure, Spring

def _spring_element_matrix(xi, yi, xj, yj, k: float) -> np.ndarray:
    dx = xj - xi
    dy = yj - yi
    ...
    # 4x4 oriented axial spring/stiffness matrix
    ke = k * np.array([
        [ c*c,  c*s, -c*c, -c*s],
        [ c*s,  s*s, -c*s, -s*s],
        [-c*c, -c*s,  c*c,  c*s],
...
def assemble_K_F(struct: Structure) -> tuple[np.ndarray, np.ndarray, dict[int,int]]:
    ...
    for sp in struct.springs:
        ...
        ke = _spring_element_matrix(ni.x, ni.y, nj.x, nj.y, sp.k)
...
def apply_dirichlet_bc(K: np.ndarray, F: np.ndarray, fixed_dofs: list[int]) -> tuple[np.ndarray, np.ndarray]:
    # Robust simple method: zero row/col, set diag=1, set force=0
    K2 = K.copy()
    F2 = F.copy()
...
def solve_displacements(struct: Structure) -> tuple[np.ndarray, dict[int, tuple[float,float]]]:
    ...
    try:
        ...
        raise np.linalg.LinAlgError(
            "System not solvable (singular). Check supports/connectivity."
        ) from e

    # Map back to node -> (ux, uy)
    disp = {}
